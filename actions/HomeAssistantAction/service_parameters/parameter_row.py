...
class ParameterRow:
    """
    Row to display service call parameters with a check button.
    """
    def __init__(self, action_core, field_name: str):
        ...
        is_active = self.field_name in self.action.settings.get_service_parameters()
    ...
    def _on_change_check(self, _) -> None:
        if self.check.get_active():
            self.action.settings.set_service_parameter(self.field_name, self.get_parameter_value())
        else:
            self.action.settings.remove_service_parameter(self.field_name)
