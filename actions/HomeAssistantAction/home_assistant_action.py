...
from de_gensyn_HomeAssistantPlugin.actions.HomeAssistantAction.helper import icon_helper, text_helper
...
class HomeAssistantAction(ActionBase):
    ...
    def _init_service_group(self) -> None:
        """
        Get all service rows.
        """
        self.service_call_service: ExpanderRow = ExpanderRow(
            self, const.SETTING_SERVICE_CALL_SERVICE, const.DEFAULT_SERVICE_CALL_SERVICE,
            title=const.LABEL_SERVICE_CALL_SERVICE, show_enable_switch=True,
            on_change=self._reload, can_reset=False, complex_var_name=True
        )

        self.service_service_combo: ComboRow = ComboRow(
            self, const.SETTING_SERVICE_SERVICE, const.EMPTY_STRING, [],
            const.LABEL_SERVICE_SERVICE, enable_search=True,
            on_change=self._on_change_service, can_reset=False,
            complex_var_name=True
        )

        self.service_parameters: ExpanderRow = ExpanderRow(
        ...
        self._service_group = self._create_group(
            const.LABEL_SETTINGS_SERVICE, [self.service_call_service.widget]
        )

    def _init_icon_group(self) -> None:
        """
        Get all icon rows.
        """

        # Icon show icon
        self.icon_show_icon: ExpanderRow = ExpanderRow(
            self, const.SETTING_ICON_SHOW_ICON, const.DEFAULT_ICON_SHOW_ICON,
            title=const.LABEL_ICON_SHOW_ICON, show_enable_switch=True,
            on_change=self._reload, can_reset=False, complex_var_name=True
        )

        # Icon icon
        self.icon_icon: EntryRow = EntryRow(
            self, const.SETTING_ICON_ICON, const.EMPTY_STRING,
            title=const.LABEL_ICON_ICON, on_change=self._reload, can_reset=False,
            complex_var_name=True
        )

        # Icon icon color
        self.icon_color: ColorButtonRow = ColorButtonRow(
            self, const.SETTING_ICON_COLOR, const.DEFAULT_ICON_COLOR,
            title=const.LABEL_ICON_COLOR, on_change=self._reload,
            can_reset=False, complex_var_name=True
        )

        # Icon scale
        self.icon_scale: ScaleRow = ScaleRow(
            self, const.SETTING_ICON_SCALE, const.DEFAULT_ICON_SCALE,
        ...
        # Icon opacity
        self.icon_opacity: ScaleRow = ScaleRow(
            self, const.SETTING_ICON_OPACITY, const.DEFAULT_ICON_OPACITY,
        ...
        icon_custom_icon_add.connect(
            const.CONNECT_CLICKED,
            self._on_add_customization, const.CUSTOMIZATION_TYPE_ICON,
            self._add_custom_icon
        )

        self.icon_custom_icon_expander: ExpanderRow = ExpanderRow(
            self, const.EMPTY_STRING, False,
        ...
        self._icon_group = self._create_group(
            const.LABEL_SETTINGS_ICON, [self.icon_show_icon.widget]
        )

    def _init_text_group(self) -> None:
        """
        Get all text rows.
        """

        # Text show text
        self.text_show_text: ExpanderRow = ExpanderRow(
            self, const.SETTING_TEXT_SHOW_TEXT, const.DEFAULT_TEXT_SHOW_TEXT,
        ...
        self.text_position_combo: ComboRow = ComboRow(
            self, const.SETTING_TEXT_POSITION, const.TEXT_POSITION_CENTER,
            text_position_model, const.LABEL_TEXT_POSITION,
            on_change=self._reload, can_reset=False, complex_var_name=True
        )

        # Text attribute
        self.text_attribute_combo: ComboRow = ComboRow(
            self, const.SETTING_TEXT_ATTRIBUTE, const.DEFAULT_TEXT_ATTRIBUTE,
            [], const.LABEL_TEXT_ATTRIBUTE, on_change=self._reload,
            can_reset=False, complex_var_name=True
        )

        # Text round
        self.text_round: ExpanderRow = ExpanderRow(
            self, const.SETTING_TEXT_ROUND, const.DEFAULT_TEXT_ROUND,
            title=const.LABEL_TEXT_ROUND, show_enable_switch=True,
            on_change=self._reload, can_reset=False, complex_var_name=True
        )

        # Text round precision
        self.text_round_precision: ScaleRow = ScaleRow(
            self, const.SETTING_TEXT_ROUND_PRECISION, const.DEFAULT_TEXT_ROUND_PRECISION,
        ...
        # Text size
        self.text_text_size: ScaleRow = ScaleRow(
            self, const.SETTING_TEXT_TEXT_SIZE, const.DEFAULT_TEXT_TEXT_SIZE,
        ...
        # Text color
        self.text_text_color: ColorButtonRow = ColorButtonRow(
            self, const.SETTING_TEXT_TEXT_COLOR, const.DEFAULT_TEXT_TEXT_COLOR,
            title=const.LABEL_TEXT_TEXT_COLOR, on_change=self._reload,
            can_reset=False, complex_var_name=True
        )

        # Text outline size
        self.text_outline_size: ScaleRow = ScaleRow(
            self, const.SETTING_TEXT_OUTLINE_SIZE, const.DEFAULT_TEXT_OUTLINE_SIZE,
            const.TEXT_OUTLINE_MIN_SIZE, const.TEXT_OUTLINE_MAX_SIZE,
            title=const.LABEL_TEXT_OUTLINE_SIZE, step=1, digits=0,
            on_change=self._reload, can_reset=False, complex_var_name=True
        )

        # Text outline color
        self.text_outline_color: ColorButtonRow = ColorButtonRow(
            self, const.SETTING_TEXT_OUTLINE_COLOR, const.DEFAULT_TEXT_OUTLINE_COLOR,
            title=const.LABEL_TEXT_OUTLINE_COLOR, on_change=self._reload,
            can_reset=False, complex_var_name=True
        )

        # Text show unit
        self.text_show_unit: SwitchRow = SwitchRow(
            self, const.SETTING_TEXT_SHOW_UNIT, const.DEFAULT_TEXT_SHOW_UNIT,
            title=const.LABEL_TEXT_SHOW_UNIT, on_change=self._reload,
            can_reset=False, complex_var_name=True
        )

        # Text unit line break
        self.text_unit_line_break: SwitchRow = SwitchRow(
            self, const.SETTING_TEXT_UNIT_LINE_BREAK, const.DEFAULT_TEXT_UNIT_LINE_BREAK,
        ...
        text_custom_text_add.connect(
            const.CONNECT_CLICKED,
            self._on_add_customization, const.CUSTOMIZATION_TYPE_TEXT,
            self._add_custom_text
        )

        self.text_custom_text_expander: ExpanderRow = ExpanderRow(
            self, const.EMPTY_STRING, False,
        ...
        self._text_group = self._create_group(
            const.LABEL_SETTINGS_TEXT, [self.text_show_text.widget]
        )
    ...
    def _load_icon_settings(self) -> None:
    ...
    def _add_custom_icon(
        self, attribute: str, operator: str, value: str, icon: str,
        color: Tuple[int, int, int, int], scale: int, opacity: int, index: int
    ) -> None:
        customization = IconCustomization(attribute, operator, value, icon, color, scale, opacity)
        custom_icons = self.settings.get_icon_customizations().copy()

        if index > -1:
            # we have to check for duplicates without the item being edited because it may have not changed
            custom_icons.pop(index)

        if customization in custom_icons:
            if index > -1:
    ...
    def _add_custom_text(
        self, attribute: str, operator: str, value: str, position: str,
        text_attribute: str, custom_text: str, text_round: bool, round_precision: int,
        text_size: int, text_color: Tuple[int, int, int, int], outline_size: int,
        outline_color: Tuple[int, int, int, int], show_unit: bool, line_break: bool, index: int
    ) -> None:
        customization = TextCustomization(
            attribute, operator, value, position, text_attribute, custom_text, text_round,
            round_precision, text_size, text_color, outline_size, outline_color,
            show_unit, line_break
        )
        custom_texts = self.settings.get_text_customizations().copy()

        if index > -1:
            # we have to check for duplicates without the item being edited because it may have
            # not been changed
            custom_texts.pop(index)

        if customization in custom_texts:
            if index > -1:
                # edited item is identical to existing - delete it
    ...
    def _update_icon(self, show_icon: bool, state: dict) -> None:
        """
    ...
    def _load_attributes(self) -> None:
        ...
        if attribute not in attribute_model:
    ...
    def _load_custom_icons(self) -> None:
        ...
        for index, customization in enumerate(self.settings.get_icon_customizations()):
            row = CustomizationIconRow(
                self.lm, customization, len(self.settings.get_icon_customizations()), index,
                attributes, state, self.settings
            )
            row.edit_button.connect(
                const.CONNECT_CLICKED, self._on_add_customization,
                const.CUSTOMIZATION_TYPE_ICON, self._add_custom_icon, index
            )
            row.delete_button.connect(
                const.CONNECT_CLICKED, self._on_delete_customization,
                const.CUSTOMIZATION_TYPE_ICON, index
            )
            row.up_button.connect(
                const.CONNECT_CLICKED, self._on_move_up,
                const.CUSTOMIZATION_TYPE_ICON, index
            )
            row.down_button.connect(
                const.CONNECT_CLICKED, self._on_move_down,
                const.CUSTOMIZATION_TYPE_ICON, index
            )
            self.icon_custom_icon_expander.add_row(row)

    def _load_custom_text(self) -> None:
        self.text_custom_text_expander.clear_rows()
        attributes = self._get_current_attributes()
        state = self.plugin_base.backend.get_entity(self.settings.get_entity())

        for index, customization in enumerate(self.settings.get_text_customizations()):
            row = CustomizationTextRow(
                self.lm, customization, len(self.settings.get_text_customizations()), index,
                attributes, state, self.settings
            )
            row.edit_button.connect(
                const.CONNECT_CLICKED, self._on_add_customization,
                const.CUSTOMIZATION_TYPE_TEXT, self._add_custom_text, index
            )
            row.delete_button.connect(
                const.CONNECT_CLICKED, self._on_delete_customization,
                const.CUSTOMIZATION_TYPE_TEXT, index
            )
            row.up_button.connect(
                const.CONNECT_CLICKED, self._on_move_up,
                const.CUSTOMIZATION_TYPE_TEXT, index
    ...
    def _on_add_customization(self, _, customization_type: str, callback, index: int = -1):
        attributes = self._get_current_attributes()
        current = None
        if index > -1:
        ...
        if customization_type == const.CUSTOMIZATION_TYPE_ICON:
            window = CustomizationIconWindow(self.lm, attributes, callback, current=current, index=index)
        elif customization_type == const.CUSTOMIZATION_TYPE_TEXT:
            window = CustomizationTextWindow(self.lm, attributes, callback, current=current, index=index)
        else:
    ...
    def _get_current_attributes(self) -> List[str]:
        ...
        """
        return [
            str(self.text_attribute_combo.get_item_at(i))
            for i in range(self.text_attribute_combo.get_item_amount())
        ]